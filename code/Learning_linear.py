...
#------------------------------------------------------------
#linear_regressionの実装
class linear_regression():
    def __init__(self,x,y):
        self.xData = x
        self.yData = y

        self.xDim = self.xData.shape[1]-1
        self.xNum = self.xData.shape[0] 

        self.yNum = self.yData.shape[0]

    def train(self):
     
        Dim = self.xData.shape[0]
     
        x_reshape = []

        x_reshaped_parts = np.array([])
     
        for i in range(Dim):
     
            x_reshape_parts = self.xData[i].reshape(-1,)
            x_reshape.append(x_reshape_parts)
            #pdb.set_trace()

        y_reshape = self.yData.reshape(-1,).astype(float) 
        
        N = len(x_reshape[0])
        G = np.array([x_reshape[0],x_reshape[1],x_reshape[2],x_reshape[3],x_reshape[4],x_reshape[5],np.ones(N)]).T
        G = G.astype(float)
        result = LA.solve(G.T.dot(G),G.T.dot(y_reshape))
...
if __name__ == "__main__":
    ...
    #linear_regression
    for no in range(len(fileind)):
        regression = linear_regression(mytrackData.train_xData[no],mytrackData.train_tData[no])
        result.append(regression.train())
    ...
    print("time : {0}".format(end_time) + "[sec]")
    f_li = open("li_w_list.binaryfile","wb")
    pickle.dump(result,f_li)
    f_li.close()
